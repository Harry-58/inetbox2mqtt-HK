...
#
#  Änderungen HK:
#    Anpassungen für display_status
#    Status/LED alive geändert
#


from tools import calculate_checksum, set_led, toggle_led
import inetboxapp
import logging
...
# Same approach for the raw PID 0xD8. This corresponds to a PID 0x18
    ...
    DISPLAY_STATUS_PIDS = (0x20, 0x61, 0xE2) # 20,21,22
    ...
    def _send_answer(self, databytes):
        ...
        self.serial.flush()
        if self.debug:
            print("out > ", databytes.hex(" "))
        if not self.d8_alive:
            toggle_led("D8")  # nur blinken wenn keine Verbindung
    ...
    def _answer_tl_request(self):
        if len(self.ts_response_buffer):
            ...
            if self.debug:
                print("unexpacted behavior - nothing to send")
    ...
    def display_status(self, line):
        self.app.handle_message(line[2] & 0x3F, line[3:])
        if line[2] == 0xE2:  # PID 0x22
            self.app.status["spannung"][0] = round((line[3] / 10.0), 1)
...
#             print()
    ...
    def assemble_cpp_buffer(self):
        ...
        if buf[:8] != self.BUFFER_PREAMBLE:
            if self.debug:
                print("buffer preamble doesn't match")
            return False
        buf_id = buf[8:10]
        self.cpp_buffer[buf_id] = buf[10:]
        if self.debug:
            print(f"Buf[{buf_id}]={self.cpp_buffer[buf_id]}")
...
# Same approach for the raw PID 0xD8. This corresponds to a PID 0x18
            if self.d8_alive:
                ...
                set_led("D8", True)  # info:HK eingefügt
            else:
    ...
    def loop_serial(self):
        ...
        if not (self.serial.any()):
            return
        # dtoggle_led("D8")  #info:HK macht keinen Sinn
        ...
        raw_pid = line[2]
        # info:HK macht hier keinen Sinn, da Daten noch fehlen. Besser am Ende
...
# send requested answer to 0x3d -> 0x7d with parity) but only, if I have the need to answer
        ...
        #    self.display_status()  #info:HK ans Ende verlegt
...
# multi-frame receive for buffer download from CPplus
        ...
        if (line[:4] == buf_trans_id) and (line[4] in range(0x21, 0x27)):
            if self.debug:
                print("Buffer-check:", line.hex("-"))
            self.cpp_in_buffer[line[4] - 0x21] = line[5:-1]  # fill into buffer-segment
            if self.debug:
                print(self.cpp_in_buffer[line[4] - 0x21].hex("*"), line[4] - 0x21)
            if (line[4] == 0x26):
...
# attention: this are raw frames with checksum -> see specification for details
        ...
        if not (cmd in cmd_ctrl.keys()):
            if self.debug:
                print(line.hex(' '), "-> no processing")
            # info:HK  Displaystatus
            if raw_pid in self.DISPLAY_STATUS_PIDS:
                self.display_status(line)
